...
namespace OrbitMap.API.Services.Implement
{
    public class VideoService : BaseService<VideoService>, IVideoService
    {
        ...
        public async Task<string> CreateVideoTimeLapse(CreateStoryTimeLapseRequest request)
        {
            ...
            double displayDuration = 0.2;
            int fps = 60;

            var tempFolder = Path.Combine(Path.GetTempPath(), "timelapse", Guid.NewGuid().ToString());
            Directory.CreateDirectory(tempFolder);

            using HttpClient httpClient = new HttpClient();
            httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/5.0 (Windows NT 10.0; Win64; x64)");

            var downloadTasks = request.MediaUrls.Select(async (url, index) =>
            {
                var response = await httpClient.GetAsync(url);
                response.EnsureSuccessStatusCode();
                var bytes = await response.Content.ReadAsByteArrayAsync();
                var filePath = Path.Combine(tempFolder, $"img{index:D4}.jpg");
                await File.WriteAllBytesAsync(filePath, bytes);
                return filePath;
            }).ToArray();

            string[] imagePaths = await Task.WhenAll(downloadTasks);
            ...
            using (var writer = new StreamWriter(fileListPath))
            {
                foreach (var path in imagePaths)
                {
                    ...
                    await writer.WriteLineAsync($"file '{formattedPath}'");
                    await writer.WriteLineAsync($"duration {displayDuration}");
                }

                // FFmpeg requires the last image to be repeated (without a duration line)
                await writer.WriteLineAsync($"file '{imagePaths.Last().Replace("\\", "/")}'");
            }

            var outputFile = Path.Combine(@"C:\Videos", $"timelapse_{DateTime.Now:yyyyMMddHHmmss}.mp4");

            // Khai báo FFmpeg trên VPS
            FFmpeg.SetExecutablesPath(@"C:\ffmpeg\bin");

            var conversion = FFmpeg.Conversions.New()
                .AddParameter(
                    $"-f concat -safe 0 -i \"{fileListPath}\" -r {fps} -c:v libx264 -pix_fmt yuv420p \"{outputFile}\"");

            await conversion.Start();

            Directory.Delete(tempFolder, true);

            var uploadResponse = await UploadTimelapseVideo(outputFile);
        ...
        //         var downloadTasks = request.MediaUrls.Select(async (url, index) =>
        //         {
        //             var imagePath = Path.Combine(workingDir, $"image_{index:000}.jpg");
        //             await DownloadImageAsync(url, imagePath);
        //         }).ToList();
        //         await Task.WhenAll(downloadTasks);
        //
        //         var outputFile = Path.Combine(workingDir, "output.mp4");
        //
        //         await CreateVideoWithFFmpegAsync(workingDir, outputFile);
        //
        //         var response = await UploadTimelapseVideo(outputFile);
        //         return response;
        //     }
        ...
        //     catch (Exception ex)
        //     {
        //         _logger.Error($"Error downloading image from URL: {url}. Exception: {ex.Message}");
        //         throw;
        //     }
        // }
        //
        // private async Task CreateVideoWithFFmpegAsync(string imageDir, string outputPath)
        // {
        ...
        //         {
        //             FileName = "ffmpeg",
        //             Arguments = $"-framerate 60 -pattern_type glob -i \"{imageDir}/image_*.jpg\" " +
        //                         $"-c:v libx264 -pix_fmt yuv420p -preset ultrafast " +
        //                         $"-filter:v \"fade=t=in:st=0:d=0.1,fade=t=out:st=2.9:d=0.1\" " +
        //                         $"-y {outputPath}",
