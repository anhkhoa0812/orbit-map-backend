...
public class StoryService : BaseService<StoryService>, IStoryService
{
    ...
    public StoryService(IUnitOfWork<OrbitMapContext> unitOfWork, ILogger logger, IMapper mapper, IHttpContextAccessor httpContextAccessor, IUploadService uploadService) : base(unitOfWork, logger, mapper, httpContextAccessor)
    {
    ...
    public async Task<NoContentResult> DeleteStoryAsync(string username, Guid id)
    {
        ...
        var story = await _unitOfWork.GetRepository<Story>().SingleOrDefaultAsync(
            predicate: x => x.Id.Equals(id) && x.User.Username.Equals(username)
        );
