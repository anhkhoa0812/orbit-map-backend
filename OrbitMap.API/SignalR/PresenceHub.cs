...
public class PresenceHub : Hub
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        var isOnline = await _tracker.UserConnected(username, Context.ConnectionId); //Cập nhập trạng thái online cho user
        if (isOnline)
        {
            ...
            foreach (var friendUsername in friendUsernames)
            {
                ...
                {
                    await Clients.Clients(connectionId).SendAsync("UserIsOnline", user);
                }
            }
            // var user = _mapper.Map<UserDto>(userEntity);
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        if (isOffline)
        {
            ...
            foreach (var friendUsername in friendUsernames)
            {
                ...
                if (connectionId != null)
                {
                    await Clients.Clients(connectionId).SendAsync("UserIsOffline", username);
                }
            }
        }
    ...
    private async Task<List<UserDto>> GetUsersOnlineAsync(string currentUsername, string[] userOnline)
    {
        ...
        var userEntities = await _unitOfWork.GetRepository<User>().GetListAsync(
    ...
    private async Task<List<string>> GetFriendUserNameOfUserAsync(User user)
    {
        ...
        var friendUsernames = friends.Select(f => f.RequesterId == user.Id ? f.Addressee.Username : f.Requester.Username).ToList();
