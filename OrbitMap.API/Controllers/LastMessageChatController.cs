...
public class LastMessageChatController : BaseController<LastMessageChatController>
{
    ...
    public async Task<ApiResult<IPaginate<LastMessageChatResponse>>> GetLastMessageChat([FromQuery] int page, [FromQuery] int size)
    {
        var currentUsername = User.GetUsername();
        _logger.Information($"BEGIN: {nameof(GetLastMessageChat)} - {DateTime.UtcNow}");
        var result = await _lastMessageChatService.GetLastMessageChat(page, size, currentUsername);
        _logger.Information($"END: {nameof(GetLastMessageChat)} - {DateTime.UtcNow}");
